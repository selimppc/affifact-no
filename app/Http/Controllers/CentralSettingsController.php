    ...
    public function update(Request $request, $id)
    {
        ...
        try {
                if($model->id==2)
                {
                    ...
                    {
                        $sender_email_id = DB::table('sender_email')->where('status', '=', 'public')->get(array('sender_email.id'));
                        foreach ($sender_email_id as $val) {
                            DB::table('sender_email')->where('id', '=', $val->id)->update(array('max_email_send' => $input['status']));
                        }
                    }
                }
